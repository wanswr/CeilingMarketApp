...
class MapEngine {
  ...
  private notifyThrottleTimer: NodeJS.Timeout | null = null;
  private cachedSortedOrders: Order[] | null = null;
  private lastStoreWriteCount: number = -1;
  ...
  private getOrdersArray = (): Order[] => {
    ...
    // Memoization based on store write count
    const currentWrites = this.entityStore.meta.writes;
    if (this.cachedSortedOrders && this.lastStoreWriteCount === currentWrites) {
        return this.cachedSortedOrders;
    }

    const orders = this.entityStore.getAllOrders()
      .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());

    this.cachedSortedOrders = orders;
    this.lastStoreWriteCount = currentWrites;
